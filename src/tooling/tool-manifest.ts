...
const verifiedPostingSchema = objectSchema({
  sourceUrl: { type: "string", minLength: 1 },
  company: { type: "string", minLength: 1 },
  title: { type: "string", minLength: 1 },
  fullText: { type: "string", minLength: 1 },
  contentCompleteness: { enum: ["full", "partial"] },
  locations: { type: "array", items: { type: "string" } },
  requiredSkills: { type: "array", items: { type: "string" } },
  preferredSkills: { type: "array", items: { type: "string" } },
  responsibilities: { type: "array", items: { type: "string" } },
  employmentTypes: { type: "array", items: { type: "string" } },
  experienceRequirement: { type: "object" },
  postedAt: { type: "string" },
  expiresAt: { type: "string" },
  status: { enum: ["open", "closed", "unknown"] },
}, ["sourceUrl", "company", "title", "fullText", "contentCompleteness"]);
...
export const JOBLENS_TOOL_DEFINITIONS: readonly JobLensToolDefinition[] = [
  ...
  { name: "joblens_discovery_hit_get", description: "Read one discovery hit by stable hitId before verification or materialization.", approvalClass: "READ_ONLY", inputSchema: objectSchema({ hitId: { type: "string", minLength: 1 } }, ["hitId"]) },
  { name: "joblens_materialize_hit", description: "Promote one durable discovery hit into a canonical JobPosting only after a client has fetched and supplied the actual posting page content. Search snippets alone are insufficient.", approvalClass: "LOCAL_MUTATION", inputSchema: objectSchema({ hitId: { type: "string", minLength: 1 }, verifiedPosting: verifiedPostingSchema }, ["hitId", "verifiedPosting"]) },
  ...
  { name: "joblens_rank", description: "Evaluate persisted canonical jobs and persist JobEvaluation and Opportunity state. Unmaterialized discovery hits are excluded.", approvalClass: "LOCAL_MUTATION", inputSchema: objectSchema({ opportunityIds: { type: "array", items: { type: "string" } } }) },
